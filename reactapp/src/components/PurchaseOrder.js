...
function PurchaseOrder() {
  ...
  const fetchPurchaseOrders = async () => {
    try {
      const response = await axios.get('https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/purchase-order');
  ...
  const handleAddPurchase = async (event) => {
    ...
    try {
      await axios.post('https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/purchase-order', newPurchase);
      fetchPurchaseOrders();
      resetForm();
    } catch (error) {
      console.error(error);
  ...
  const handleUpdatePurchase = async (event) => {
    ...
    try {
      await axios.put(`https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/purchase-order/${editOrderId}`, updatedPurchase);
      fetchPurchaseOrders();
      resetForm();
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteProduct = async (orderId) => {
    try {
      await axios.delete(`https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/purchase-order/${orderId}`);
      fetchPurchaseOrders();
    } catch (error) {
      console.error(error);
    }
