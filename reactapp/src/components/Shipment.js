...
function Shipment() {
  ...
  const fetchShipmentOrders = async () => {
    try {
      const response = await axios.get('https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/shipment');
  ...
  const handleAddShipment = async (event) => {
    ...
    try {
      await axios.post('https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/shipment/post', newShipment);
      fetchShipmentOrders();
      resetForm();
    } catch (error) {
      console.error(error);
  ...
  const handleUpdateShipment = async (event) => {
    ...
    try {
      await axios.put(`https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/shipment/update/${editOrderId}`, updatedShipment);
      fetchShipmentOrders();
      resetForm();
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteShipment = async (orderId) => {
    try {
      await axios.delete(`https://8080-bdffaeaffaddeebcaddaceaeaadbdbabf.project.examly.io/shipment/delete/${orderId}`);
      fetchShipmentOrders();
    } catch (error) {
      console.error(error);
    }
